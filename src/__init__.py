...
WITHOUT_FINISHED_PLUGIN_ID = "upload_clear_without_finished"
...
class Plugin(BasePlugin):
    ...
    def _install_menu_item(self):
        ...
        if self._menu is menu and self._layout_installed and "Cancelled / Failed / User Logged Off" in item_labels:
            self._install_attempts = 0
            return
        ...
        self._rebuild_clear_menu(
            uploads_view,
            menu,
            include_finished_options=self._has_finished_clear_options(item_labels),
            inactive_callback=self.on_clear_cancelled_failed_logged_off,
        )
        self._menu = menu
        self._layout_installed = True
        self._install_attempts = 0
    ...
    def _is_without_finished_enabled(self):
        try:
            return WITHOUT_FINISHED_PLUGIN_ID in self.parent.enabled_plugins
        except Exception:
            return False

    def _has_finished_clear_options(self, item_labels):
        return bool(item_labels.intersection({
    ...
    def _restore_clear_menu(self):
        ...
        self._rebuild_clear_menu(
            uploads_view,
            menu,
            include_finished_options=not self._is_without_finished_enabled(),
            inactive_callback=None,
        )
    ...
    def on_clear_cancelled_failed_logged_off(self, *_args):
        core.uploads.clear_uploads(statuses={
